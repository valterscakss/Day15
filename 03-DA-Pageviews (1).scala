...
// MAGIC However...
// MAGIC 
// MAGIC You _can_ parse it with two slightly ugly workarounds. First, you need to use the Spark SQL `unix_timestamp` function, along with
...
// MAGIC into a Unix time (a long integer representing the number of milliseconds since January 1, 1970)
// MAGIC 
// MAGIC Then, you can do one of two things:
// MAGIC 
// MAGIC 1. Divide that result by 1000.0, which produces _seconds_ in a format that _can_ be cast to a timestamp. This approach is compatible
// MAGIC    with Hive, but it loses milliseconds-precision. (We don't care, in this case, since the timestamps are only granular to seconds.)
// MAGIC    
// MAGIC 2. Convert the Unix time _back_ to a string, with `from_unixtime`. The resulting string is in a format that Spark SQL can cast to
// MAGIC    a timestamp.
// MAGIC    
// MAGIC In this instance, we'll use option 1.
...
//pageviewsDF.select("requests").sum()  //.select(prependfNumberUDF($"Day of week"), $"total requests")

pageviewsDF.agg(sum("requests")).first.get(0)//("requests").sum().show()
...
val vv = pageviewsDF.filter($"site" === "mobile")
vv.agg(sum("requests")).first.get(0)// Type your answer here...
...
val bb = pageviewsDF.filter($"site" === "desktop")
bb.agg(sum("requests")).first.get(0)
...
pageviewsTimestampDF.select(month($"timestamp")).distinct().show()// Type your answer here...
...
val bb = pageviewsDF.filter($"site" === "mobile")
//val eu = bb.select(second($"timestamp")).show()

//ee.avg($"requests").first.get(0)
//val uu = eu.avg(second($"timestamp"))($"requests").first.get(0)
// val dis = bb.groupBy((second($"requ"),avg(0)))
val transactions_with_counts = bb
  .groupBy(second($"timestamp"), $"requests")
  .avg()
  .show()
